...
from models import (
    ...
    # --- MODELOS DE SAÍDA IMPORTADOS ---
    FuncionarioOut, ProjetoOut, TarefaOut, CalendarioOut
)
...
# --- CRUDs (existentes) ---
@app.post("/funcionarios", response_model=FuncionarioOut, tags=["Funcionários"])
...
@app.get("/funcionarios/me", response_model=FuncionarioOut, tags=["Funcionários"])
async def ler_usuario_logado(current_user: Funcionario = Depends(auth.get_usuario_logado)):
    return current_user

@app.get("/funcionarios", response_model=List[FuncionarioOut], tags=["Funcionários"])
...
@app.get("/funcionarios/{id}", response_model=FuncionarioOut, tags=["Funcionários"])
...
@app.put("/funcionarios/{id}", response_model=FuncionarioOut, tags=["Funcionários"])
...
async def criar_projeto(projeto_data: ProjetoCreate, current_user: Funcionario = Depends(auth.get_usuario_logado)):
    ...
    # Retorna o objeto 'projeto' que ainda tem o 'responsavel' populado na memória
    return projeto
...
@app.get("/projetos/{id}", response_model=ProjetoOut, tags=["Projetos"])
...
@app.put("/projetos/{id}", response_model=ProjetoOut, tags=["Projetos"])
async def atualizar_projeto(id: PydanticObjectId, update_data: ProjetoUpdate, current_user: Funcionario = Depends(auth.get_usuario_logado)):
    ...
    await projeto.save()
    # Re-busca com fetch_links=True para garantir que a resposta está totalmente populada
...
@app.post("/tarefas", response_model=TarefaOut, tags=["Tarefas"])
async def criar_tarefa(tarefa_data: TarefaCreate, current_user: Funcionario = Depends(auth.get_usuario_logado)):
    ...
    await tarefa.insert()
    # Retorna o objeto 'tarefa' que ainda tem 'projeto' e 'responsavel' populados
    return tarefa
...
@app.get("/tarefas/{id}", response_model=TarefaOut, tags=["Tarefas"])
...
@app.put("/tarefas/{id}", response_model=TarefaOut, tags=["Tarefas"])
...
@app.post("/calendario", response_model=CalendarioOut, tags=["Calendário"])
async def criar_evento_calendario(calendario_data: CalendarioCreate, current_user: Funcionario = Depends(auth.get_usuario_logado)):
    ...
    if calendario_data.projeto_id:
        projeto_link = await Projeto.get(PydanticObjectId(calendario_data.projeto_id), fetch_links=True) # Fetch links para popular o ProjetoOut
    ...
    if calendario_data.tarefa_id:
        tarefa_link = await Tarefa.get(PydanticObjectId(calendario_data.tarefa_id), fetch_links=True) # Fetch links para popular o TarefaOut
        if not tarefa_link: raise HTTPException(status_code=404, detail="Tarefa para agendamento não encontrada.")
    
    evento_dict = calendario_data.dict(exclude={"projeto_id", "tarefa_id"})
    evento = Calendario(**evento_dict, projeto=projeto_link, tarefa=tarefa_link)
    await evento.insert()
    # Retorna o objeto 'evento' que tem os links populados na memória
    return evento

@app.get("/calendario", response_model=List[CalendarioOut], tags=["Calendário"])
...
@app.get("/calendario/{id}", response_model=CalendarioOut, tags=["Calendário"])
...
@app.put("/calendario/{id}", response_model=CalendarioOut, tags=["Calendário"])
async def atualizar_evento_calendario(id: PydanticObjectId, update_data: CalendarioUpdate, current_user: Funcionario = Depends(auth.get_usuario_logado)):
    ...
    if "projeto_id" in update_dict:
        if update_dict["projeto_id"]:
            evento.projeto = await Projeto.get(PydanticObjectId(update_dict["projeto_id"]))
        else:
            evento.projeto = None
            
    if "tarefa_id" in update_dict:
        if update_dict["tarefa_id"]:
            evento.tarefa = await Tarefa.get(PydanticObjectId(update_dict["tarefa_id"]))
        else:
            evento.tarefa = None
